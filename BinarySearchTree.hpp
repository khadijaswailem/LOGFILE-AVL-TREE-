...
public:
    ...
    int getCount();
    NODE* getRoot();
    NODE* rangeSearch(ITEM low, ITEM high, ITEM key, NODE* n);
...
template <class NODE, typename ITEM> NODE* BinarySearchTree<NODE, ITEM>::rangeSearch(ITEM low, ITEM high, ITEM key, NODE* n)
{
    if (this->contains(key) == false) {
        return NULL;//key isnt even in tree
    }
    if (n == NULL) {
        return NULL;
    }
    else if (n->getItem() == key) {
        return n;//if key is in the root
    }
    else if (n->getItem() > high) {
        return rangeSearch(low, high, key, n->getLeft());//search on left side bcs our val is less than root
    }
    else if (n->getItem() < low) {
        return rangeSearch(low, high, key, n->getRight());//search on right bcs our val is bigger than root
    }
    else {
        return rangeSearch(low, high, key, n);//key is within those bounds so will call function 
